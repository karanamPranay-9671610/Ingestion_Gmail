...
def LUMPER(text, file, filename):
   ...
   """
    ...
    record = data
    rec = mydatabase.LUMPER.insert_one(record)
    # filename = filename.rsplit('.', maxsplit=1)[0]
    df = pd.DataFrame.from_dict(data, orient='index').T
    df['uuid'] = str(filename)
    # df['filepath'] = str(file)
    headers = ['load_number', 'date', 'ship_from_address','ship_to_address', 'transaction_fee', 'amount',
               'total_paid', 'po_number', 'receipt_number', 'work_date', 'carrier', 'vendor', 'trailer_number',
               'total_payments', 'uuid']
    for i in headers:
        if i not in df.columns:
            df[i] = ''
    df = df[headers]
    df.to_csv('lumper.csv', index=False)

    sql = """
    INSERT INTO pdfextraction.lumper (
    load_number, date, ship_from_address, ship_to_address,
    transaction_fee, amount, total_paid, po_number,
    receipt_number, work_date, carrier, vendor,
    trailer_number, total_payments, uuid
        )
        VALUES (
            %s, %s, %s, %s,
            %s, %s, %s, %s,
    ...
    # Open the CSV file and read data
        ...
        print("Failed to insert Lumper Data into SQL DataBase Error from Lumper Module.")
    return data